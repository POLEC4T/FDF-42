...
/*   Updated: 2025/03/03 16:05:23 by mniemaz          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		t_map *map, char **str_line_elems)
{
	...
	if (strtab_len(str_elem) == 2)
	{
		...
		map->colors[elem_pos.row][elem_pos.col] = int_to_rgb(ft_atoui_base(
					ft_str_toupper(str_elem[1]) + 2, HEX_BASE));
	}
	else
	{
		map->heights[elem_pos.row][elem_pos.col] = ft_atoi_improved(
				str_line_elems[i_str_line_elems], &is_number_out_of_range);
...
static void	exit_error_init_map(char *msg, char **lines, t_map *map)
{
	free(lines);
	free_map_vars(map, 0);
	exit_perror(msg);
...
void	init_map(t_map *map, char *filename)
{
	...
	if (!lines)
		exit_error_init_map("Error alloc", lines, map);
	fd = open(filename, O_RDONLY);
	if (fd < 0)
		exit_error_init_map("Error opening file", lines, map);
	row_elems = -1;
